...
@Slf4j
public class PauseSurveyUseCase implements UseCase<PauseSurveyUseCase.Input, SurveyOutput> {
  ...
  public SurveyOutput execute(Input input) {
    ...
    if (List.of(SurveyLifecycle.ENDED, SurveyLifecycle.PAUSED).contains(survey.getLifecycle())) {
    ...
    surveyStateTransitionsRepository.record(
        SurveyStateTransition.record(
            survey.id(), before, survey.stateAt(now, window), TransitionReason.MANUAL_PAUSE, now));

    log.info("Pesquisa pausada [{}] application={}", survey.id().value(), input.applicationId());
