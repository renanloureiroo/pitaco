...
@Slf4j
public class ResumeSurveyUseCase implements UseCase<ResumeSurveyUseCase.Input, SurveyOutput> {
  ...
  public SurveyOutput execute(Input input) {
    ...
    if (List.of(SurveyLifecycle.ENDED, SurveyLifecycle.PUBLISHED).contains(survey.getLifecycle())) {
    ...
    surveyStateTransitionsRepository.record(
        SurveyStateTransition.record(
            survey.id(), before, survey.stateAt(now, window), TransitionReason.MANUAL_RESUME, now));

    log.info("Pesquisa retomada [{}] application={}", survey.id().value(), input.applicationId());
